...
screencast() {
   wf-recorder --audio=$SOUND_CARD -f ~/dl/screencast.mkv
}

area() {
   wf-recorder --audio=$SOUND_CARD -g "$(slurp)" -f ~/dl/screencast.mkv
}
...
share() {
    ...
    curl -F "file=@~/dl/screencast.mkv" https://0x0.st | wl-copy && hyprctl notify 2 5000 "rgb(153276)" "fontsize:15  Video stored in 0x0.st"
}
...
kill_proc(){
    ...
    if [ $? -eq 0 ];
    then
		hyprctl notify 2 5000 "rgb(153276)" "fontsize:15 Video was stored in ~/dl/screencast.mkv"
        pkill --signal SIGINT wf-recorder
        exit 0
    fi
}
...
remove_vid() {
    [ -f ~/dl/screencast.mkv ] && rm ~/dl/screencast.mkv
}
...
case $OPT in
    ...
    'area')
        ...
        area;;
    'share') 
        check_connection && share || hyprctl notify 3 5000 "rgb(ff5566)" "fontsize:15 Video was stored in ~/dl/screencast.mkv";;
    *|quit) exit 0;;
esac
